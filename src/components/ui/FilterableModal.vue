                ...
                <div class="modal-container">
                            ...
                            <div class="grid grid-cols-12 gap-6">
                                <div class="col-span-6" v-for="(item, index) in filterables.columns" :key="index">
                                    ...
                                    <template v-if="item.type === 'text'">
                                        <generic-input type="text" :placeholder="item.placeholder" :label="item.label"
                                            v-model="state[item.column]" :disabled="loadingState[item.column]" />
                                    </template>

                                    <!-- Select -->
                                    <template v-if="item.type === 'select'">
                                        <custom-select :options="optionsState[item.column] || []" :label="item.label"
                                            :name="item.column" :placeholder="item.placeholder"
                                            :disabled="loadingState[item.column]" v-model:value="state[item.column]"
                    ...
                    <div class="modal-footer">
                        <div style="margin-right: 15px;">
                            <custom-button label="Chiudi" styleType="secondary-outline" @onClick="emitCloseModal" />
                        </div>

                        <div style="margin-right: 15px;">
                            <custom-button label="Pulisci" styleType="secondary" @onClick="cleanResults" />
                        </div>

                        <custom-button label="Cerca" styleType="primary" @onClick="emitFilter" />
                    </div>
                </div>
            </div>
        </div>
